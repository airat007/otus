...
Перем Запись_НастройкиГлобальные Экспорт;
Перем Запись_НастройкиТопика Экспорт;

Перем Чтение_НастройкиГлобальные Экспорт;
Перем Чтение_НастройкиТопика Экспорт; 

Перем Просмотр_НастройкиГлобальные Экспорт;

Перем Адрес_Запись Экспорт;
Перем Топик_Запись Экспорт;
Перем Раздел_Запись Экспорт; 

Перем тзТопикиРазделы_Чтение Экспорт;
Перем тзТопики_Чтение Экспорт; 
Перем тзДанные_Чтение Экспорт;

Перем чслПрочитатьКоличество_Чтение Экспорт;
Перем чслТаймаутПоУмолчанию Экспорт;

Перем GroupID_Чтение Экспорт; 
Перем Адрес_Чтение Экспорт; 
...
Процедура ЗаполнитьТопикиРазделы(КафкаТопик)
	
	стрДобавить = тзТопикиРазделы_Чтение.Добавить();
	стрДобавить.Topic = КафкаТопик;
...
Процедура ЗаполнитьПоУмолчанию(КафкаТопик) Экспорт
	
	КафкаАдрес = Константы.КафкаАдрес.Получить(); 
	КафкаПолучатель = Константы.КафкаПолучатель.Получить();
	СоздатьТаблицыЗначений();
	
	Адрес_Чтение = КафкаАдрес;
	GroupID_Чтение = КафкаПолучатель;
	
	
	чслПрочитатьКоличество_Чтение = 100; //1000
	чслТаймаутПоУмолчанию = 10000;       //3000
	
	Адрес_Запись = КафкаАдрес;
	Топик_Запись = КафкаТопик;
	Раздел_Запись = -1;  
	
	Адрес_Просмотр = КафкаАдрес;          
	GroupID_Просмотр = КафкаПолучатель;

	
	ЗаполнитьНастройкиГлобальные();
	ЗаполнитьНастройкиТопика();
	ЗаполнитьТопикиРазделы(КафкаТопик);
...
Процедура СоздатьТаблицыЗначений() 
	...
	тзТопикиРазделы_Чтение.Колонки.Добавить("Partition", Новый ОписаниеТипов("Число"));
	тзТопикиРазделы_Чтение.Колонки.Добавить("Offset", Новый ОписаниеТипов("Число"));

	тзТопики_Чтение = Новый ТаблицаЗначений();
	тзТопики_Чтение.Колонки.Добавить("Topic", );

	тзДанные_Чтение = Получатель_ПолучитьТаблицуСообщения();
